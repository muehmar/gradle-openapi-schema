...
public class ListMemberMappingWriter {
  ...
  public static Writer fullAutoListMemberMappingWriter(
  ...
  public static class WrapListFieldBuilder {
    ...
    static Function<JavaPojoMember, Writer> wrapTristateList(FlagFieldNameScope nameScope) {
      return member ->
          javaWriter()
              .print(
                  "l -> Tristate.ofNullableAndNullFlag(l, %s)", member.getIsNullFlagName(nameScope))
              .ref(OpenApiUtilRefs.TRISTATE);
    }

    static Function<JavaPojoMember, Writer> autoWrapList(FlagFieldNameScope nameScope) {
      return member -> {
        if (member.isRequiredAndNotNullable()) {
          ...
          return wrapTristateList(nameScope).apply(member);
        }
      };
    }
  }
  ...
  public static Writer build(ListMemberMappingWriter mappingWriter) {
    if (isIdentityWriter(mappingWriter.mapListItemType.apply(mappingWriter.member))
        && isIdentityWriter(mappingWriter.wrapListItem.apply(mappingWriter.member))
        && isIdentityWriter(mappingWriter.mapListType.apply(mappingWriter.member))
        && isIdentityWriter(mappingWriter.wrapList.apply(mappingWriter.member))) {
      return javaWriter()
          .print(
              "%s%s%s",
