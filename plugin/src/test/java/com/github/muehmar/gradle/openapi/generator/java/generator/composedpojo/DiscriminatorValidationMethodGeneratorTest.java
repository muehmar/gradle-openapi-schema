...
class DiscriminatorValidationMethodGeneratorTest {
  ...
  void generate_when_calledWithDiscriminator_then_correctOutput(ComposedPojo.CompositionType type) {
    final Generator<JavaComposedPojo, PojoSettings> generator =
        DiscriminatorValidationMethodGenerator.generator();
    final Writer writer =
        generator.generate(
  ...
  void generate_when_protectedAndDeprecatedValidationSettings_then_correctOutput() {
    ...
    final Writer writer =
        generator.generate(
            JavaPojos.composedPojoWithDiscriminator(ComposedPojo.CompositionType.ONE_OF),
            TestPojoSettings.defaultSettings()
