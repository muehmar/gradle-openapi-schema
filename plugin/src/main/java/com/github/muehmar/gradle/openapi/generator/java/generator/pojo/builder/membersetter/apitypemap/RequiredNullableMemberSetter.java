...
class RequiredNullableMemberSetter implements MemberSetter {
  ...
  public RequiredNullableMemberSetter(JavaPojoMember member, JavaMapType javaMapType) {
    this.member = member;
  ...
  public static Optional<MemberSetter> fromMember(JavaPojoMember member) {
    return member
        .getJavaType()
        .onMapType()
        .map(javaMapType -> new RequiredNullableMemberSetter(member, javaMapType));
  }

  @Override
  public boolean shouldBeUsed(PojoSettings settings) {
    return ApiTypeMapConditions.groupCondition().test(member) && member.isRequiredAndNullable();
  }
