...
class OptionalNullableMemberSetter implements MemberSetter {
  ...
  public OptionalNullableMemberSetter(JavaPojoMember member, JavaMapType javaMapType) {
    this.member = member;
  ...
  public static Optional<MemberSetter> fromMember(JavaPojoMember member) {
    return member
        .getJavaType()
        .onMapType()
        .map(javaMapType -> new OptionalNullableMemberSetter(member, javaMapType));
  }

  @Override
  public boolean shouldBeUsed(PojoSettings settings) {
    return ApiTypeMapConditions.groupCondition().test(member) && member.isOptionalAndNullable();
  }
